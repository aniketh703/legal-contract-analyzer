...
InLegalBERT (fine-tuned) & \textbf{85.6\%} & 0.708 & \textbf{0.847} \\
\bottomrule
\end{tabular}
\end{table}

InLegalBERT was fine-tuned for 4 epochs on a single Colab T4 GPU (Section~\ref{sec:bert-finetune}), reaching accuracy 0.8557 and weighted-F1 0.8470 against the baseline's 0.8247 and 0.8238 -- a clear win on both aggregate metrics, and the more relevant pair for a production system where each prediction is equally likely regardless of class. Macro-F1, which weights all 47 classes equally, tells a different story: InLegalBERT's 0.708 is \emph{below} the baseline's 0.740. Inspecting the per-class report, InLegalBERT scores exactly 0.00 F1 on five classes, all among the thinnest in the training set: \textit{AffiliateLicenseLicensee} (support 8), \textit{DPDP} (support 3), \textit{IrrevocableOrPerpetualLicense} (support 5), \textit{NoSolicitOfCustomers} (support 10), and \textit{UnlimitedAllYouCanEatLicense} (support 3). The TF-IDF baseline, despite lower overall accuracy, degrades more gracefully on these same long-tail classes. We read this as four fine-tuning epochs being enough to fit the majority classes well but not enough exposure to the rare ones for a 110M-parameter model to generalize past memorizing the majority-class prior -- a concrete illustration of why we report macro- and weighted-F1 separately rather than accuracy alone.
...
\textbf{Compute accessibility.} The InLegalBERT fine-tune's $>$12-hour CPU-only estimate versus its measured 28-minute cost on a free-tier Colab T4 GPU (4 epochs, 1{,}940 optimizer steps) is a small but concrete data point on the accessibility of legal-domain transformer fine-tuning for resource-constrained teams, a population we expect to include most legal-tech developers building India-specific tools.

\textbf{Accuracy is not the whole story for a 47-class production system.} InLegalBERT's macro-F1 regression (Section~\ref{sec:results}) would be invisible to anyone who validated the fine-tune on accuracy alone, since accuracy is dominated by the well-represented classes it handles well. A classifier that is silently useless on five clause types -- two of them (\textit{DPDP}, a 2023 Indian data-protection statute) among the most consequential for compliance risk -- is a worse production choice than the aggregate numbers suggest, despite winning on every aggregate metric except macro-F1. We would not have caught this without deliberately reporting macro-F1 alongside accuracy, which reinforces the paper's broader argument that evaluation choices, not just model choices, determine what a result actually means.

\section{Conclusion and Future Work}
\label{sec:conclusion}

We presented a statute-grounded contract risk analysis pipeline for Indian commercial contracts, combining a three-stage clause classifier with a three-layer hybrid retriever over the Indian Contract Act, 1872. We showed that a deterministic clause-type-to-statute map, not learned retrieval, accounts for the retriever's near-perfect Hit@5, and we reported a real evaluation-harness leakage bug together with its fix, arguing that this correction is itself a reportable contribution. We constructed a clean, leakage-free 47-class benchmark on which a fine-tuned InLegalBERT classifier beats a TF-IDF baseline on accuracy (85.6\% vs.\ 82.5\%) and weighted-F1 (0.847 vs.\ 0.824) but loses on macro-F1 (0.708 vs.\ 0.740), failing completely on several of the thinnest classes -- a result we take as evidence that model comparisons for imbalanced, many-class legal taxonomies should report class-balanced metrics by default, not as an afterthought. Future work includes completing human annotation of the real-world evaluation set, testing the cross-jurisdiction transfer assumption directly, addressing InLegalBERT's thin-class failures (class-weighted loss, targeted data augmentation, or more epochs with early stopping per class), and, contingent on that work, wiring the stronger classifier into the production cascade.
