...
fixed_function = r'''def assess_document(
    text: str,
...
# We will truncate the file exactly where the first `def assess_document` starts.
idx = content.find("def assess_document(")
if idx != -1:
    new_content = content[:idx] + fixed_function
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Repaired successfully.")
else:
    print("Could not find def assess_document(")
